...
# Reset the migration-053 chain anchors so each test sees an empty chain.
# TRUNCATE on the protected tables clears their rows but leaves the anchor's
# ``latest_row_hash`` pointing at a row that no longer exists; the next INSERT
# would then chain off a deleted row's hash, breaking verification semantics
# for that test.  An UPDATE back to the seeded NULLs keeps the contract clean.
RESET_AUDIT_CHAIN_ANCHORS_SQL = """
UPDATE audit_chain_anchors
   SET latest_row_id = NULL,
       latest_row_hash = NULL,
       row_count = 0,
       updated_at = now()
"""

# Deterministic 32-byte test secret for the audit chain.  Real deployments
# use a random secret from ``AUDIT_CHAIN_SECRET``; tests need a stable value
# so a chain built in one transaction can be verified in another.
TEST_AUDIT_CHAIN_SECRET_HEX = "00" * 32
...
async def _reset_db(session) -> None:
    ...
    await session.execute(text(RESET_AUDIT_CHAIN_ANCHORS_SQL))
...
async def pg_uow(test_session_factory):
    ...
    async with test_session_factory() as session:
        ...
        # Establish the audit-chain GUC before any INSERT into a protected
        # table; without it migration 053's trigger raises.  Set after
        # ``_reset_db`` because the commit there clears transaction-local GUCs.
        await set_audit_chain_secret(session, TEST_AUDIT_CHAIN_SECRET_HEX)
        uow = SqlAlchemyUnitOfWork(session, audit_chain_secret_hex=TEST_AUDIT_CHAIN_SECRET_HEX)
        try:
            yield uow
        finally:
            await _reset_db(session)
