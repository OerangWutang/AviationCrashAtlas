...
def _set_prod_env(monkeypatch, **extra):
    ...
    # dummy value so production-mode tests reach the check they exercise.
    monkeypatch.setenv("AUDIT_CHAIN_SECRET_HEX", _PROD_SECRET)
    # Migration 054 makes MFA_KEK mandatory in production for the same reason.
    monkeypatch.setenv("MFA_KEK_HEX", _PROD_SECRET)
