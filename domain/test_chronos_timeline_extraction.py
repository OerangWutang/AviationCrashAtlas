...
@pytest.mark.asyncio
async def test_extracts_impact_time_from_event_date_and_local_time():
    uow, event_id = _make_uow({"event_date": "2009-02-12", "occurred_time_local": "2217"})
    result = await ExtractChronosTimelineFromEvent(uow).execute(event_id)
    assert result.timeline_events_created_count == 1
    events = await uow.chronos_timeline_events.list_for_accident_event(event_id)
    assert len(events) == 1
    assert events[0].event_type == ChronosTimelineEventType.IMPACT
    assert events[0].raw_value == "2009-02-12T22:17"
    assert events[0].timestamp_precision == ChronosTimestampPrecision.MINUTE


@pytest.mark.asyncio
async def test_extracts_impact_day_from_event_date_when_local_time_missing():
    uow, event_id = _make_uow({"event_date": "2009-02-12"})
    result = await ExtractChronosTimelineFromEvent(uow).execute(event_id)
    assert result.timeline_events_created_count == 1
    events = await uow.chronos_timeline_events.list_for_accident_event(event_id)
    assert len(events) == 1
    assert events[0].event_type == ChronosTimelineEventType.IMPACT
    assert events[0].raw_value == "2009-02-12"
