...
def test_no_egg_info_in_source_tree() -> None:
    ...
    # Filter out egg-info dirs that are already gitignored — they cannot leak
    # into a release commit.
    unignored = [
        d
        for d in egg_info_dirs
        if subprocess.run(
            ["git", "check-ignore", str(d.relative_to(REPO_ROOT))],
            capture_output=True,
            cwd=REPO_ROOT,
        ).returncode != 0
    ]
    assert not unignored, (
        f"Found build artifact(s) in src/ that are NOT gitignored: {unignored}. "
        f"Remove them with: rm -rf src/*.egg-info"
    )
...
def _make_settings(**overrides: Any):
    ...
    base = {
        ...
        # Migration 053 requires a chain HMAC secret in production; supply a
        # dummy so tests reach the validation they actually exercise.
        "audit_chain_secret_hex": "a" * 64,
        # Migration 054 requires an MFA KEK in production for the same reason.
        "mfa_kek_hex": "a" * 64,
