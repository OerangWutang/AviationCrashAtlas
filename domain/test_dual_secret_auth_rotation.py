...
class _FakeResult:
    ...
    def scalar_one(self):
        # Used by the lockout pre-check, which counts failed attempts;
        # zero keeps the test focused on the dual-secret behaviour.
        return 0
...
async def test_auth_accepts_hash_with_previous_secret(monkeypatch):
    ...
    request = SimpleNamespace(
        url=SimpleNamespace(path="/api/v1/accidents"),
        method="GET",
        headers={},
