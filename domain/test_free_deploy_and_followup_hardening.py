...
def _read_orm_text() -> str:
    ...
    db = ROOT / "src/atlas/infrastructure/db"
    files = sorted(db.glob("_orm_*.py")) + [db / "orm_models.py"]
...
def test_outbox_hot_path_indexes_are_declared_in_orm_and_migration() -> None:
    orm = read("src/atlas/infrastructure/db/_orm_projections.py")
...
def test_lockfiles_are_marked_for_python_312_runtime() -> None:
    # Lockfiles are generated by `uv pip compile --python-version 3.12` (see the
    # `lock` target in the Makefile). uv echoes its full command line into the
    # file header, so the resolved-for-3.12 marker is the flag itself. This pins
    # the resolution to the production runtime regardless of the host interpreter.
    for lockfile in ("requirements.txt", "requirements-dev.txt"):
        header = read(lockfile)
        assert "autogenerated by uv" in header, (
            f"{lockfile} must be generated by `uv pip compile` (run `make lock`)."
        )
        assert "--python-version 3.12" in header, (
            f"{lockfile} must be resolved for Python 3.12 (run `make lock`)."
        )
...
def test_production_rejects_short_prometheus_bearer_token(monkeypatch) -> None:
    ...
    monkeypatch.setenv("AUDIT_CHAIN_SECRET_HEX", "a" * 64)
    monkeypatch.setenv("MFA_KEK_HEX", "a" * 64)
    monkeypatch.setenv("ALLOWED_HOSTS", "api.example.com")
    monkeypatch.setenv("API_DOCS_ENABLED", "false")
    monkeypatch.setenv("SECURITY_HEADERS_ENABLED", "true")
