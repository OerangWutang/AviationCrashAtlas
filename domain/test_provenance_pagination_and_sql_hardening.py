...
_DB_PKG = Path("src/atlas/infrastructure/db")


def _read_orm_text() -> str:
    """Concatenated source of orm_models.py and all _orm_*.py sub-modules."""
    files = sorted(_DB_PKG.glob("_orm_*.py")) + [_DB_PKG / "orm_models.py"]
...
def test_projection_history_uniqueness_uses_single_covering_index() -> None:
    orm_text = (_DB_PKG / "_orm_projections.py").read_text()
    migration_text = Path("alembic/versions/021_provenance_pagination_indexes.py").read_text()
...
# ── Archive retrieval (soft-delete) ─────────────────────────────────────────────
#
# Completes the retention feature: provenance hides retention-swept claims
# (deleted_at IS NOT NULL) by default and re-surfaces them when
# include_archive=True. Conflict-history accepts the flag but it is a no-op
# there (those rows carry no deleted_at column).


async def _seed_event_with_swept_claim() -> tuple[InMemoryUnitOfWork, object, Claim, Claim]:
    uow = InMemoryUnitOfWork()
    event = AccidentEvent(id=uuid4())
    source = Source(id=uuid4(), name="NTSB", kind=SourceKind.EXTERNAL)
    uow._store.events[event.id] = event
    uow._store.sources[source.id] = source
    live = Claim(
        event_id=event.id,
        source_id=source.id,
        field_name="operator",
    ...
    uow._store.claims[live.id] = live
    uow._store.claims[swept.id] = swept
    return uow, event, live, swept


async def test_provenance_hides_swept_claims_by_default() -> None:
    uow, event, live, swept = await _seed_event_with_swept_claim()

    result = await QueryProvenance(uow).execute(event.id)

    ids = {c["id"] for c in result["claims"]}
    assert live.id in ids
    assert swept.id not in ids
    # A hidden swept claim exists, so clients are told the archive has content.
    assert result["archive_available"] is True


async def test_provenance_includes_swept_claims_when_requested() -> None:
    uow, event, live, swept = await _seed_event_with_swept_claim()

    result = await QueryProvenance(uow).execute(event.id, include_archive=True)

    by_id = {c["id"]: c for c in result["claims"]}
    assert live.id in by_id
    assert swept.id in by_id
    # The surfaced row carries its tombstone so callers can flag it as archived.
    assert by_id[swept.id]["deleted_at"] is not None


async def test_provenance_archive_available_false_when_nothing_swept() -> None:
    uow = InMemoryUnitOfWork()
    event = AccidentEvent(id=uuid4())
    source = Source(id=uuid4(), name="NTSB", kind=SourceKind.EXTERNAL)
    uow._store.events[event.id] = event
    ...
    assert {c["id"] for c in result["claims"]} == {live.id}
    assert result["archive_available"] is False


async def test_conflict_history_include_archive_is_noop() -> None:
    uow = InMemoryUnitOfWork()
    conflict = ClaimConflict(
        event_id=uuid4(),
        field_name="operator",
    ...
    uow._store.conflicts[conflict.id] = conflict

    plain = await QueryConflictHistory(uow).execute(conflict.id)
    archived = await QueryConflictHistory(uow).execute(conflict.id, include_archive=True)

    # No deleted_at on conflict-activity rows: the flag changes nothing.
    assert archived["archive_available"] is False
    assert plain["transitions"] == archived["transitions"]
