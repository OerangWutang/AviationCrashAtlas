...
def _authed(app, user: CurrentUser) -> AsyncClient:
    ...
    async def _stamp(request):
        ...
        # MFA gate: tests assume the role/schema layer is what gets exercised.
        # We bypass MFA here so a missing X-MFA-Code header doesn't shadow the
        # 422/403 we're actually trying to assert.  MFA behaviour itself is
        # covered by tests/domain/test_mfa_primitives.py and integration tests.
        app.dependency_overrides[dependencies.require_mfa] = lambda: user
