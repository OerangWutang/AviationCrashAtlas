...
class ClaimRepository(ABC):
    ...
    async def find_all_by_event(
        ...
        *,
        ...
        """Return claims for an event, keyset-paginated by ``(created_at, id)``.

        ``include_deleted`` controls retention-swept rows: by default
        soft-deleted claims (``deleted_at IS NOT NULL``) are excluded so the
        retention tombstone is honored. Pass ``include_deleted=True`` to surface
        them — this backs the ``include_archive`` provenance flag.
        """
        ...

    @abstractmethod
    async def has_archived_claims(self, event_id: UUID) -> bool:
        """Return whether the event has any soft-deleted (retention-swept) claims.

        Lets provenance report ``archive_available`` accurately without paging
        the full archive: a cheap ``EXISTS`` over ``deleted_at IS NOT NULL``.
        """
        ...
    ...
    async def find_active_by_source_and_field(
        ...
        """Return all active claims from a source with the given field_name.

        Used for bulk cleanup of claims produced under an old field name
        (e.g. ``occurred_on`` → ``event_date``) after re-ingesting with a
        renamed importer mapping.
        """
        ...
