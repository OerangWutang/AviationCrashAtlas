...
# Confidence scores are 0.0–1.0 across all AI sub-engines.
...
class Claim(DomainModel):
    ...
    # Retention tombstone (ComplianceMixin column). Set by RetentionSweep when a
    # claim's retention deadline passes; None for live claims. Surfaced here so
    # provenance can hide swept claims by default and re-include them when the
    # caller passes include_archive=True.
...
class ClaimHistory(DomainModel):
    ...
    created_at: datetime = Field(default_factory=utc_now)
    # HMAC audit chain columns — set by DB trigger, never by application code.
    # Stored as hex strings so they serialise directly to JSON.
    row_hash: str | None = None
    prev_hash: str | None = None

    @field_validator("row_hash", "prev_hash", mode="before")
    @classmethod
    def _bytes_to_hex(cls, v: Any) -> str | None:
        if v is None:
            return None
...
class ClaimConflict(DomainModel):
    ...
    *current state* of the last modification, which on creation is simply the
    ...
    @field_validator("winning_claim_id", mode="after")
    @classmethod
    def winning_claim_id_not_cleared_on_resolved(
        cls, v: UUID | None, info: Any
    ) -> UUID | None:
        if v is None and info.data.get("status") == ConflictStatus.RESOLVED:
            raise ValueError(
                "winning_claim_id cannot be set to None on a RESOLVED conflict"
            )
        return v
