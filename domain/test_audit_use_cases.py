...
class TestGetPublicEventAudit:
    ...
    async def test_summary_describes_field_count(self) -> None:
        ...
        assert "1 field drawn" in result.summary

    async def test_summary_uses_singular_verbs_for_one_dispute_and_override(self) -> None:
        uow = InMemoryUnitOfWork()
        event = _seed_event(uow)
        _seed_projection(uow, event.id, fields={"operator": "__DISPUTED__"}, unresolved=["operator"])
        _seed_published_page(uow, event.id, "singular-summary")
        source = _seed_source(uow)
        _seed_claim(
            uow,
            event.id,
        ...
        result = await GetPublicEventAudit(uow).execute("singular-summary")
        assert "1 field is currently disputed" in result.summary
        assert "1 field was set by an editor" in result.summary
