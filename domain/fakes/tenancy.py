...
class FakeTenantMembershipRepository(TenantMembershipRepository):
    ...
    async def add(self, membership: TenantMembership) -> None:
        # Mirror the DB-level (tenant_id, atlas_user_id) uniqueness.
        for m in self._s.memberships:
            if m.tenant_id == membership.tenant_id and m.atlas_user_id == membership.atlas_user_id:
    ...
    async def get_for_user_in_tenant(
        ...
        for m in self._s.memberships:
            if m.tenant_id == tenant_id and m.atlas_user_id == user_id:
                return m
        return None
