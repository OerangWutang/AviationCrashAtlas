...
class FakeIngestionRunRepository:
    ...
    async def update_status(self, id: UUID, status: str, finished_at: object | None = None) -> None:
        ...
        run.finished_at = (
            ...
            else (datetime.now(UTC) if status in {"FINISHED", "FAILED", "finished", "completed", "failed"} else None)
        )
...
class FakeClaimRepository:
        ...
        *,
        ...
        items = sorted(
            (
                ...
                if c.event_id == event_id and (include_deleted or c.deleted_at is None)
            ),
            key=lambda c: (c.created_at, str(c.id)),
        )
        return _slice_after_id(items, after_id, limit)

    async def has_archived_claims(self, event_id: UUID) -> bool:
        return any(
            c.event_id == event_id and c.deleted_at is not None
            for c in self._s.claims.values()
        )
    ...
    async def find_active_by_source_and_field(
        self, source_id: UUID, field_name: str
    ) -> list[Claim]:
        return [
            c
            for c in self._s.claims.values()
            if c.source_id == source_id and c.field_name == field_name and c.is_active
        ]
...
class FakeConflictRepository:
        ...
        *,
        ...
        for stored in self._s.conflicts.values():
            if stored.event_id != event_id or stored.status != ConflictStatus.OPEN:
                continue
            # Set SYSTEM_AUTO_CLOSED before status so the model validator's
            # RESOLVED+winning_claim_id=None check sees the exemption flag.
            # Do NOT re-assign winning_claim_id=None — OPEN conflicts already
            # have it None, and assigning it after status=RESOLVED fires the
            # field validator which rejects None on a RESOLVED conflict.
    ...
    async def update_with_version_check(
        ...
        if c is None or c.version != expected_version:
            return None
        # Field-assignment order matters because ClaimConflict.validate_assignment
        # fires a cross-field invariant on every setattr.
        # OPEN→RESOLVED: set winning_claim_id BEFORE status=RESOLVED (status last).
        # RESOLVED→OPEN: set status=OPEN BEFORE clearing winning_claim_id (status first).
        new_status = updates.get("status")
        resolving = new_status in (ConflictStatus.RESOLVED, ConflictStatus.RESOLVED.value)
        opening = new_status in (ConflictStatus.OPEN, ConflictStatus.OPEN.value)

        def _order(kv: tuple) -> int:
            if kv[0] != "status":
                return 0
            if resolving:
                return 1   # status last when resolving
            if opening:
                return -1  # status first when reopening
            return 0

        ordered_items = sorted(updates.items(), key=_order)
