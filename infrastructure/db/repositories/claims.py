...
class SqlClaimRepository(ClaimRepository):
    ...
    async def find_all_by_event(
        ...
        *,
        ...
        if not include_deleted:
            # Honor the retention tombstone: swept claims are hidden unless the
            # caller explicitly asks for the archive (include_archive=True).
            stmt = stmt.where(ClaimModel.deleted_at.is_(None))
    ...
    async def has_archived_claims(self, event_id: UUID) -> bool:
        stmt = select(
            ...
            .where(
                ClaimModel.event_id == event_id,
                ClaimModel.deleted_at.is_not(None),
            )
            .exists()
        )
    ...
    async def find_active_by_source_and_field(
        ...
        result = await self._session.execute(
            ...
            .order_by(ClaimModel.created_at.asc(), ClaimModel.id.asc())
        )
        return [_to_domain(obj, Claim) for obj in result.scalars()]
