...
class SqlAlchemyUnitOfWork(UnitOfWork):
    def __init__(self, session: AsyncSession, audit_chain_secret_hex: str | None = None):
        self.session = session
        # When set, commit/rollback re-establishes the audit.chain_secret
        # GUC so subsequent statements in the same session can still
        # write to chained audit tables.  See ``set_audit_chain_secret``.
        self._audit_chain_secret_hex = audit_chain_secret_hex
    ...
    async def commit(self) -> None:
        ...
        if self._audit_chain_secret_hex is not None:
            await set_audit_chain_secret(self.session, self._audit_chain_secret_hex)
...
class SqlAlchemyTenantUnitOfWork(SqlAlchemyUnitOfWork):
    ...
    def __init__(
        ...
        super().__init__(session, audit_chain_secret_hex=audit_chain_secret_hex)
...
#: GUC read by the ``audit_chain_append`` trigger (migration 053).  The trigger
#: HMACs each new row against the prior row's hash using this secret; without it
#: set the trigger raises and the INSERT fails, by design.
AUDIT_CHAIN_GUC = "audit.chain_secret"
...
async def set_audit_chain_secret(session: AsyncSession, secret_hex: str) -> None:
    """Bind the audit-chain HMAC secret for the current transaction.

    Mirrors :func:`set_tenant_context`: transaction-local ``set_config`` so the
    secret cannot leak across PgBouncer-pooled connections.  The
    ``audit_chain_append`` trigger reads this GUC and refuses to insert if it is
    unset or shorter than 16 bytes, so callers must establish it before any
    chained-table write inside the transaction.
    """
    await session.execute(
        text("SELECT set_config(:k, :v, true)"),
        {"k": AUDIT_CHAIN_GUC, "v": secret_hex},
    )
...
async def create_tenant_uow(tenant_id: uuid.UUID) -> AsyncIterator[SqlAlchemyUnitOfWork]:
    ...
    async with async_tenant_session_factory() as session:
        uow = SqlAlchemyTenantUnitOfWork(session, tenant_id, audit_chain_secret_hex=chain_hex)
        try:
