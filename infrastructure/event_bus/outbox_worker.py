...
from atlas.domain.enums import OutboxEventType, OutboxStatus
...
class OutboxWorker:
    ...
    async def _mark_failed(self, event: OutboxEvent, error: str) -> None:  # pragma: no cover
        ...
        if new_status == OutboxStatus.DEAD_LETTER and event.event_type == OutboxEventType.ECHO_CROSSREF_REQUESTED:
    ...
    async def recover_stale_locks(self) -> int:
        ...
        for event in deadlettered:
            if event.event_type == OutboxEventType.ECHO_CROSSREF_REQUESTED:
    ...
    async def _process_event(self, uow, event) -> bool:
        ...
        if event.event_type != OutboxEventType.CLAIMS_UPDATED:
