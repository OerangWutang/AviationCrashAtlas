...
#
# --no-deps is required, not just an optimisation: requirements.txt is a complete,
# fully-pinned dependency closure produced by `uv pip compile`, so re-resolving is
# unnecessary. It is also load-bearing here — prometheus-fastapi-instrumentator 7.1.0
# declares starlette<1.0.0, but the lock intentionally pins starlette==1.1.0 (the
# PYSEC-2026-161 fix) via the [tool.uv] override. Letting pip re-resolve would reject
# that pin; --no-deps installs the verified closure verbatim.
