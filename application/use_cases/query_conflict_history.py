...
class QueryConflictHistory:
    ...
    async def execute(
        ...
        *,
        ...
        # ``include_archive`` is accepted for API symmetry with provenance but is
        # intentionally a no-op here: conflict-activity rows carry no retention
        # tombstone (no ``deleted_at`` column), so there is no archived history to
        # surface. ``archive_available`` therefore stays False. Accepting the flag
        # (rather than 501-ing) keeps the endpoint contract consistent.
        _ = include_archive
