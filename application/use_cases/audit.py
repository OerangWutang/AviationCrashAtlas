    ...
    *,
    ...
    if disputed_count > 0:
        parts.append(
            ...
            f"{'' if disputed_count == 1 else 's'} "
            f"{'is' if disputed_count == 1 else 'are'} currently "
            f"disputed between sources."
        )
    if manual_override_count > 0:
        parts.append(
            f"{manual_override_count} field"
            f"{'' if manual_override_count == 1 else 's'} "
            f"{'was' if manual_override_count == 1 else 'were'} set by "
            f"an editor."
        )
