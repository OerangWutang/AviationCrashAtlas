...
from atlas.domain.entities import AccidentEvent, PendingDuplicateReview, RawSnapshot, Source
from atlas.domain.enums import IngestionRunStatus
...
class _ResolveResult:
    event: AccidentEvent
    pending_reviews: list[PendingDuplicateReview]
    event_created: bool
    attached_by: str
...
def _ingestion_submission_hash_material(
    *,
    ...
    # captured_at is operational metadata (when the data was imported), not
    # submission content.  Importers that set it to now() on each run would
    # produce a new hash for every run even with identical source data.
    fingerprint.pop("captured_at", None)
...
class IngestSourceData:
    ...
    async def execute_with_result(
        ...
        early_result = await idempotency_svc.check(
            ...
            sorted_claims=hashes["sorted_claims_data"],
            source_record_id=source_record_id,
            event_id=event_id,
        )
        ...
        loaded = await self._load_and_normalise(
            source_id, event_id, ingestion_run_id, claims_data, hashes, settings
        )
        explicit_event = loaded.explicit_event
        normalised_claims = loaded.normalised_claims
        normalised_claim_fields = loaded.normalised_claim_fields
        submission_fingerprint = loaded.submission_fingerprint
        ...
        resolved = await self._resolve_event(
            source_id,
            event_id,
            explicit_event,
        ...
        pending_reviews = resolved.pending_reviews
        event_created = resolved.event_created
        attached_by = resolved.attached_by
        identity_index_maintained = resolved.identity_index_maintained
        ...
        await self._uow.ingestion_runs.update_status(
            ingestion_run_id, IngestionRunStatus.FINISHED.value, finished_at=utc_now()
        )
    ...
    def _validate_and_hash(
        ...
        # Sorted, serialized claims list from the hash material — used by the
        # idempotency content-only fallback to detect captured_at-only diffs.
        sorted_claims_data: list[dict] = submission_hash_material.get("claims_data", [])
        ...
        return {
            ...
            "source_record_id": source_record_id,
            "captured_at": captured_at,
            "sorted_claims_data": sorted_claims_data,
        }
    ...
    async def _load_and_normalise(
        ...
        return _LoadResult(
            ...
            normalised_claim_fields=normalised_claim_fields,
    ...
    async def _resolve_event(
        ...
        if explicit_event is not None:
            return _ResolveResult(explicit_event, [], False, "explicit_event_id", False)
        if existing_event_from_record_id is not None:
            return _ResolveResult(existing_event_from_record_id, [], False, "source_record_id", False)
        ...
        return _ResolveResult(event, pending_reviews, event_created, attached_by, True)
