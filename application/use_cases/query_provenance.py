    ...
    async def execute(
        ...
        *,
        ...
        claims_page = await self._uow.claims.find_all_by_event(
            ...
            include_deleted=include_archive,
        )
        ...
        # True when retention-swept claims exist for this event. Signals clients
        # that passing include_archive=True will surface additional (hidden)
        # evidence. Only claims carry a deleted_at tombstone today.
        archive_available = await self._uow.claims.has_archived_claims(canonical_event_id)
