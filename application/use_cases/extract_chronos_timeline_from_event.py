...
_HH_MM_RE = re.compile(r"^(\d{1,2}):(\d{2})$")


def _event_date_time_fallback(fields: dict[str, object]) -> tuple[str, str] | None:
    event_date = safe_str(fields.get("event_date"))
    if event_date is None:
        return None

    occurred_time = safe_str(fields.get("occurred_time_local"))
    if occurred_time is None:
        return "event_date", event_date

    time_match = _HHMM_RE.match(occurred_time) or _HH_MM_RE.match(occurred_time)
    ...
    return "occurred_time_local", f"{event_date}T{hour:02d}:{minute:02d}"
...
class ExtractChronosTimelineFromEvent:
    ...
    async def execute(self, event_id: UUID) -> ChronosExtractionResult:
        ...
        for event_type in _SEQUENCE_ORDER:
            ...
            if raw_val is None and event_type == ChronosTimelineEventType.IMPACT:
                fallback = _event_date_time_fallback(fields)
                if fallback is not None:
                    matched_field, raw_val = fallback
