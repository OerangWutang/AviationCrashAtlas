...
class IngestionIdempotencyService:
    ...
    def snapshot_hash_matches(
        ...
        *,
        ...
        Modern rows compare the explicit full-submission hash.  When that
        doesn't match, a content-only comparison is attempted: if raw_payload,
        claims, source_record_id and event_id all match, the only possible
        difference is ``captured_at`` (operational import timestamp), which is
        not part of submission identity.  Legacy rows from before submission
        fingerprints existed may have stored only the raw-payload hash in
        ``payload_hash``; those are accepted only when the new audit columns
        are absent.
        """
        ...
        # Content-only match: accept replay where only captured_at differs.
        # Importers that set captured_at=now() on each run produce a changed
        # submission_hash despite identical source data.  The content check
        # compares raw_payload (via its pre-storage hash), claims, source_record_id
        # and event_id — every field except the operational import timestamp.
        stored_fp = snapshot.submission_fingerprint_json
        if (
            snapshot.raw_payload_hash is not None
            and snapshot.raw_payload_hash == raw_payload_hash
            and stored_fp is not None
            and stored_fp.get("source_record_id") == source_record_id
            and stored_fp.get("event_id") == (str(event_id) if event_id else None)
            and stored_fp.get("claims_data") == sorted_claims
        ):
            logger.info(
                "Accepting idempotency replay where only captured_at differs; "
                "snapshot_id=%s source_record_id=%s",
                snapshot.id,
                source_record_id,
            )
            return True
