...
NTSB_FIELD_MAPPING: dict[str, str] = {
    ...
    # dec_latitude/dec_longitude are decimal degrees (WGS-84).
    # The sibling latitude/longitude columns use legacy DDMMSS encoding and are not mapped.
...
# IngestionClaimDTO rejects field_value JSON representations > 50 KB.
# NTSB narrative memos can exceed this; truncate at the claim level while
# preserving the full text verbatim in raw_payload for audit purposes.
_MAX_NARRATIVE_CHARS = 48_000
...
def _truncate_narrative(value: str | None) -> str | None:
    if value is None or len(value) <= _MAX_NARRATIVE_CHARS:
        return value
    return value[:_MAX_NARRATIVE_CHARS] + " [truncated]"
...
def build_event_record(
    *,
    ...
    put("event_date", ev_date.isoformat() if ev_date else None)
    ...
    put("latitude", _to_float(event_row.get("dec_latitude")))
    put("longitude", _to_float(event_row.get("dec_longitude")))
    ...
    # ---- narratives ------------------------------------------------------- #
    put("probable_cause_narrative", _truncate_narrative(_clean_str(narrative.get("narr_cause"))))
    put("factual_narrative", _truncate_narrative(_clean_str(narrative.get("narr_accf"))))
    put("analysis_narrative", _truncate_narrative(_clean_str(narrative.get("narr_accp"))))
