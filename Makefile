...
install:        ## Install from lockfile (run 'make lock' first if missing)
	...
	# --no-deps: requirements-dev.txt is a complete pinned closure from `uv pip compile`.
	# It is also required so pip does not re-resolve and reject the intentional
	# starlette==1.1.0 pin (PYSEC-2026-161 fix) against prometheus-fastapi-instrumentator's
	# defensive starlette<1.0.0 metadata cap. See [tool.uv] override-dependencies in pyproject.toml.
	pip install --no-deps -r requirements-dev.txt
	pip install -e . --no-deps

lock:           ## Pin dependencies via uv (generates requirements*.txt)
	@command -v uv >/dev/null 2>&1 || { echo "ERROR: uv not found. Install: https://docs.astral.sh/uv/"; exit 1; }
	# uv (not bare pip-compile) is required: it applies [tool.uv] override-dependencies
	# from pyproject.toml, which lifts prometheus-fastapi-instrumentator's defensive
	# starlette<1.0.0 cap so the PYSEC-2026-161 fix (starlette>=1.0.1) can be resolved.
	uv pip compile --python-version 3.12 --no-emit-index-url --strip-extras -o requirements.txt requirements.in
	uv pip compile --python-version 3.12 --no-emit-index-url --strip-extras -o requirements-dev.txt requirements-dev.in
	uv lock
	@echo "Lockfiles updated — commit requirements.txt, requirements-dev.txt and uv.lock"
...
audit-chain-verify: ## Verify the tamper-evident audit chain across all protected tables
	atlas audit-chain-verify
...
release-clean:  ## Remove ALL generated files; use before creating a release archive
	...
	rm -rf build/ dist/ .coverage coverage.xml htmlcov/ .venv/ .agents/ deploy/free/.env
