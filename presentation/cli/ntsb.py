...
def remediate(
    ...
    """Fix records stuck after the occurred_on→event_date field-name rename.

    Phase 0 — Update the NTSB Source row's field_mapping_json to the current
    NTSB_FIELD_MAPPING if it has drifted from a previous importer version.

    Phase 1 — Re-ingest via the corrected importer.  The idempotency key
    embeds a content hash of the submitted claims, so the renamed field
    (event_date vs occurred_on) produces a different key and a fresh
    ingestion run for each affected record.  Unchanged records replay from
    the stored result and are not re-written.

    Phase 2 — Supersede orphaned occurred_on claims.  Any still-active
    occurred_on claim that now has a corresponding event_date claim from the
    same source and event is superseded and the event queued for projection
    rebuild so completeness is recalculated.
    """
    from uuid import uuid4

    from atlas.application.ingestion import ProjectionUpdater
    from atlas.application.use_cases.ingest_source_data import IngestSourceData
    from atlas.domain.entities import Source
    from atlas.domain.enums import SourceKind
    from atlas.infrastructure.db.unit_of_work import create_uow
    ...
    async def run() -> None:
        # ── Phase 0: ensure source; sync field_mapping_json if stale ──────────
        async with create_uow() as uow:
            source = await uow.sources.get_by_name(NTSB_SOURCE_NAME)
            if source is None:
                source = Source(
                ...
                await uow.sources.add(source)
                typer.echo(
                    f"Registered source {NTSB_SOURCE_NAME!r} (id={source.id})"
                )
            else:
                typer.echo(f"Using existing source {NTSB_SOURCE_NAME!r} (id={source.id})")
                current_mapping = dict(NTSB_FIELD_MAPPING)
                if source.field_mapping_json != current_mapping:
                    await uow.sources.update_field_mapping(source.id, current_mapping)
                    typer.echo("  Updated field_mapping_json to current NTSB_FIELD_MAPPING")
        source_id = source.id

        # ── Phase 1: re-ingest through the corrected importer ─────────────────
        typer.echo("\nPhase 1: re-ingesting through corrected importer …")
        done = created = failed = 0
        for record in reader.iter_records():
            run_id = IngestSourceData.derive_ingestion_run_id(source_id, record.idempotency_key)
            try:
                ...
                failed += 1
                typer.echo(
                    f"  ! {record.source_record_id}: {type(exc).__name__}: {exc}", err=True
                )
            if progress_every and done and done % progress_every == 0:
                typer.echo(f"  … {done} ingested ({created} new events, {failed} failed)")
            if limit and done >= limit:
                break
        typer.echo(f"Phase 1 done. ingested={done} new_events={created} failed={failed}")

        # ── Phase 2: supersede orphaned occurred_on claims ────────────────────
        typer.echo("\nPhase 2: superseding orphaned occurred_on claims …")
        superseded_count = skipped_no_replacement = 0
        # Use a sentinel run id so outbox payloads are traceable.
        cleanup_run_id = uuid4()
        async with create_uow() as uow:
            orphans = await uow.claims.find_active_by_source_and_field(
                source_id, "occurred_on"
            )
            typer.echo(f"  found {len(orphans)} active occurred_on claim(s)")
            for old_claim in orphans:
                active = await uow.claims.find_active_by_event(old_claim.event_id)
                replacements = [
                    c
                    for c in active
                    if c.source_id == source_id and c.field_name == "event_date"
                ]
                if not replacements:
                    skipped_no_replacement += 1
                    continue
                await uow.claims.bulk_supersede(
                    [old_claim.id], by_claim_id=replacements[0].id
                )
                await ProjectionUpdater(uow).queue(
                    old_claim.event_id, source_id, cleanup_run_id
                )
                superseded_count += 1
        typer.echo(
            f"Phase 2 done. superseded={superseded_count} "
            f"skipped_no_replacement={skipped_no_replacement}"
        )
