...
class LegalHoldRequest(BaseModel):
    """Apply or release a legal hold on a single entity row.

    ``reason`` is required and persisted in the compliance ledger so a
    later auditor can reconstruct *why* the hold was applied — empty
    or whitespace-only reasons are rejected at the use case layer.
    """

    entity_type: EntityType = Field(
        description="Discriminator naming the table the entity_id refers to.",
    )
    entity_id: UUID = Field(description="Primary key of the row to hold or release.")
    reason: str = Field(
        min_length=1,
        max_length=2000,
        description="Free-text justification recorded in the ledger.",
    )


class LegalHoldResponse(BaseModel):
    event_id: UUID = Field(description="ID of the compliance_events ledger row written.")
    entity_type: EntityType
    entity_id: UUID
    action: Literal["LEGAL_HOLD_APPLIED", "LEGAL_HOLD_RELEASED"]


class SetRetentionRequest(BaseModel):
    """Set or clear the ``retention_until`` deadline on an entity row.

    ``retention_until=null`` clears the deadline so the retention sweeper
    leaves the row alone indefinitely.  A non-null value must be in the
    future; the use case rejects past values to avoid silently making the
    row eligible for the next sweep before the admin can review.
    """
    ...
    retention_until: datetime | None = Field(
        default=None,
        description=(
            "Timezone-aware deadline.  null clears the deadline.  Must be in the future when set."
        ),
    )
    reason: str = Field(min_length=1, max_length=2000)


class SetRetentionResponse(BaseModel):
    event_id: UUID
    entity_type: EntityType
    entity_id: UUID
...
class RedactionRequest(BaseModel):
    """Flag an entity row as redacted.

    Sets the ``redacted_at``/``redacted_by``/``redaction_reason`` columns
    and appends a ``REDACTION_APPLIED`` row to the compliance ledger.
    Downstream consumers (projections, exports) are responsible for
    masking content when ``redacted_at IS NOT NULL`` — this endpoint is
    purely the status + audit surface.
    """
...
class RedactionResponse(BaseModel):
    ...
    entity_type: EntityType
    entity_id: UUID
    action: Literal["REDACTION_APPLIED"] = "REDACTION_APPLIED"
