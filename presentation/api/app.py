...
_VERSION = importlib.metadata.version("atlas-backend")
...
def _add_middleware(app: FastAPI, settings: Settings) -> None:
    ...
    if settings.cors_origins:
        app.add_middleware(
            ...
            allow_headers=["Authorization", "Content-Type", "X-API-Key", "X-MFA-Code"],
        )
...
def _configure_otel(app: FastAPI) -> None:
    ...
    Imports are deferred so an unconfigured deploy never contacts an OTLP
    collector and packages are not required at import time.  If the env var
    is set but packages are missing the server logs a warning and continues
    without tracing — it does not abort startup.

    Instrument *after* all middleware and routers so the OTel ASGI wrapper
    is the outermost layer and captures complete request/response spans.
    """
    ...
    resource = Resource.create(
        {
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "atlas-backend"),
    ...
    logger.info("OpenTelemetry tracing active, exporting to %s", endpoint)
