...
from atlas.domain.enums import ClaimType, Role
...
@router.get(
    "/{conflict_id}/candidates",
    response_model=ConflictCandidatesResponse,
)
async def list_conflict_candidates(
    ...
    """Return the full claim candidates participating in a conflict.

    This exists so the resolve UI can submit a stable ``winning_claim_id``
    instead of indexing into an unrelated array.  The public evidence
    endpoint deliberately hides claim ids, so reviewers need an
    authenticated read that exposes them alongside the source metadata they
    actually need to make a decision (source name, reliability tier, claim
    type, timestamp, current winning/superseded state).

    Behaviour notes:

    * Claims whose ``claim_ids`` are present on the conflict but whose
      ``ClaimModel`` row no longer exists are simply omitted from the list
      (repository ``get_many`` already filters them out).
    * Claims whose ``source_id`` cannot be resolved are omitted as well: a
      candidate without source attribution cannot be reviewed defensibly.
      This should not happen against the live DB but is handled defensively
      because the in-memory test store can construct it.
    * ``is_winning`` is derived from ``conflict.winning_claim_id``; it is
      always False on OPEN conflicts.
    * ``is_superseded`` is derived from the claim's own ``claim_type``.
    * Candidates are sorted by ``(is_superseded, reliability_tier ASC,
      created_at DESC)`` so the most-trusted, most-recent, active claim is
      shown first.  Reliability tier in this codebase is "lower = more
      trusted" (see ``Source.reliability_tier``).
    """
    conflict = await uow.conflicts.get(conflict_id)
    if conflict is None:
        raise HTTPException(status_code=404, detail="Conflict not found")

    claims = await uow.claims.get_many(list(conflict.claim_ids))
    if not claims:
        await uow.rollback()
        return {
    ...
    for claim in claims:
        ...
        if source is None:
            # Defensive: a claim with no resolvable source is unreviewable.
            # Skip rather than emit a candidate with placeholder metadata
            # that the UI could mistake for a real source.
            logger.warning(
                "Conflict %s references claim %s whose source %s is missing; "
        ...
        candidates.append(
            ConflictClaimCandidateDTO(
                ...
                source_reliability_tier=source.reliability_tier,
                claim_type=claim.claim_type.value,
                created_at=claim.created_at,
                is_winning=(
                    conflict.winning_claim_id is not None
                    and claim.id == conflict.winning_claim_id
                ),
    ...
    # Stable, useful sort: active before superseded; within each group the
    # most trusted source (lowest reliability_tier) wins; ties broken by
    # most recent first; final tiebreak on claim id for determinism.
    candidates.sort(
        key=lambda c: (
            c.is_superseded,
            c.source_reliability_tier,
    ...
    return {
        "conflict_id": conflict_id,
        "field_name": conflict.field_name,
        "candidates": [_dump(c) for c in candidates],
    }
