...
from atlas.application.use_cases.legal_hold import (
    ApplyLegalHold,
    ApplyRedaction,
    ReleaseLegalHold,
    SetRetention,
...
from atlas.presentation.api.dependencies import get_uow, require_mfa, require_role
...
async def apply_legal_hold(
    ...
    """Apply a legal hold to an entity row.

    The hold disables the retention sweeper for this row until released
    via DELETE /legal-holds.  Both the column flip and the ledger row
    are written in the same transaction.
    """
    event_id = await ApplyLegalHold(uow).execute(
...
@router.delete("/legal-holds", response_model=LegalHoldResponse)
async def release_legal_hold(
    ...
    """Release a legal hold on an entity row.

    The previous LEGAL_HOLD_APPLIED ledger row is preserved — the
    ledger is append-only, so the release is recorded as a new row.
    """
    event_id = await ReleaseLegalHold(uow).execute(
        entity_type=body.entity_type,
        entity_id=body.entity_id,
        reason=body.reason,
        actor_id=current_user.user_id,
...
@router.put("/retention", response_model=SetRetentionResponse)
async def set_retention(
    ...
    """Set or clear the retention deadline for an entity row.

    Pass ``retention_until=null`` to clear the deadline so the row is
    retained indefinitely.  Non-null values must be in the future.
    """
    event_id = await SetRetention(uow).execute(
        entity_type=body.entity_type,
        entity_id=body.entity_id,
...
@router.post("/redactions", response_model=RedactionResponse, status_code=201)
async def apply_redaction(
    ...
    Idempotent: re-applying preserves the original ``redacted_at`` but
    refreshes ``redacted_by``/``redaction_reason`` and appends a fresh
    ledger row.  Actual content blanking is enforced by downstream
    consumers that respect ``redacted_at IS NOT NULL``.
    """
    event_id = await ApplyRedaction(uow).execute(
        entity_type=body.entity_type,
        entity_id=body.entity_id,
        reason=body.reason,
        actor_id=current_user.user_id,
