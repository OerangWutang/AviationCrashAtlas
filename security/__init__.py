...
def sign_evidence_hash(content_hash: str) -> str:
    """Sign a SHA-256 content hash with the evidence signing secret.

    Returns the HMAC-SHA256 digest as a hex string.  Callers store
    this alongside the hash so they can later verify the hash was
    issued by this Atlas server.  Falls back quietly to a no-op
    placeholder when no signing secret is configured (development).
    """
    secret_hex = get_settings().resolve_evidence_signing_secret_hex()
    if secret_hex is None:
        # Development fallback: sign with a hash of the content itself
        # so the field is always populated but not meaningful.
        return hashlib.sha256(f"no-secret:{content_hash}".encode()).hexdigest()
    return hmac.digest(
        bytes.fromhex(secret_hex), content_hash.encode(), hashlib.sha256
    ).hex()
