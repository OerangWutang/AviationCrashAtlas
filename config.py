...
class Settings(BaseSettings):
    ...
    # HMAC key for the per-row audit chain (migration 053).  The trigger
    # ``audit_chain_append`` reads it from the ``audit.chain_secret`` session
    # GUC, which the UoW sets on each new SQL session.  Rotating this value
    # invalidates the historical chain — only do so in a controlled window
    # alongside a re-run of ``audit_chain_backfill``.  Accepts either raw
    # bytes (any length, will be hex-encoded before being passed to the GUC)
    # or a pre-hex-encoded value when set via ``AUDIT_CHAIN_SECRET_HEX``.
    audit_chain_secret: str | None = None
    audit_chain_secret_hex: str | None = None

    # HMAC key for signing evidence upload receipts.  Every uploaded document's
    # content_sha256 is signed with this key so callers can verify the hash
    # wasn't tampered with after the fact.  Defaults to audit_chain_secret when
    # not explicitly set (so single-secret deployments still get signed receipts).
    evidence_signing_secret: str | None = None
    evidence_signing_secret_hex: str | None = None
    ...
    # ── Auth lockout + MFA (migration 054, TICKET-009) ──────────────────────
    # IP-based brute-force lockout on the API-key auth path.  After
    # ``login_lockout_threshold`` failed attempts from a single client IP
    # inside ``login_lockout_window_minutes``, the auth path short-circuits
    # with HTTP 429 + Retry-After for ``login_lockout_duration_minutes``.
    # Counting failures (not all attempts) means a legitimate caller mixed
    # with an attacker's flood is not also locked out.
    login_lockout_threshold: int = Field(default=10, gt=0, le=1000)
    login_lockout_window_minutes: int = Field(default=15, gt=0, le=1440)
    login_lockout_duration_minutes: int = Field(default=15, gt=0, le=1440)

    # TOTP MFA configuration for destructive endpoints.  ``mfa_issuer_name``
    # is the label shown in the authenticator app.  ``mfa_kek`` is the
    # key-encryption-key used by AES-256-GCM to wrap per-key TOTP seeds at
    # rest in ``api_keys.mfa_secret_encrypted``.  Production requires it
    # whenever any key has ``mfa_required=true`` — without the KEK the auth
    # path cannot verify codes, so soft-bricking is prevented at startup.
    # Accepts raw bytes or hex (mirrors the audit-chain secret pattern).
    mfa_issuer_name: str = "Atlas"
    mfa_kek: str | None = None
    mfa_kek_hex: str | None = None
    ...
    # Number of trusted reverse proxies between the internet and Atlas.
    # Governs how _client_ip() extracts the real client IP from X-Forwarded-For
    # for the IP-based auth lockout.
    #
    # 0 (default) — X-Forwarded-For is NOT trusted.  The raw TCP peer address
    #   (request.client.host) is used instead.  Safe when Atlas is directly
    #   internet-facing or when the deployment topology is unknown.  When running
    #   behind a single reverse proxy this means the lockout buckets all clients
    #   through that proxy together, which is less granular but cannot be bypassed
    #   by header injection.
    #
    # N > 0 — The Nth entry from the RIGHT of X-Forwarded-For is used.  Each
    #   trusted proxy appends the previous hop's IP, so xff[-N] is the real
    #   client IP as recorded by the outermost (most trusted) proxy.  The client
    #   can only prepend entries; it cannot forge the entries appended by trusted
    #   proxies.
    #
    # Examples (TRUSTED_PROXY_COUNT=1, single AWS ALB / nginx in front):
    #   Client spoofs:  XFF: evil-ip         → proxy adds real-ip → xff[-1] = real-ip ✓
    #   Normal request: XFF: <not set>       → proxy adds real-ip → xff[-1] = real-ip ✓
    ...
    # Host-header protection. Explicitly set in production to your domain,
    # e.g. api.example.com,localhost. Wildcard default was removed for
    # defense-in-depth — production already requires explicit hosts via
    ...
    def resolve_mfa_kek_hex(self) -> str | None:
        """Return the MFA key-encryption-key as a hex string, or None.

        Mirrors :meth:`resolve_audit_chain_secret_hex`.  Accepts either
        ``MFA_KEK_HEX`` (passed through) or ``MFA_KEK`` (raw bytes,
        hex-encoded here).  The seed-encryption helpers in
        ``atlas.security.mfa`` decode this to AES-256-GCM key material.
        """
        if self.mfa_kek_hex:
            return self.mfa_kek_hex.strip()
        if self.mfa_kek:
            return self.mfa_kek.encode("utf-8").hex()
        return None

    def resolve_audit_chain_secret_hex(self) -> str | None:
        """Return the audit chain HMAC key as a hex string, or None.

        Accepts ``AUDIT_CHAIN_SECRET_HEX`` (passed through unchanged) or
        ``AUDIT_CHAIN_SECRET`` (raw bytes, encoded here).  The trigger
        function decodes with ``decode(secret_hex, 'hex')`` so the GUC
        value must be a valid hex string at the wire.
        """
        if self.audit_chain_secret_hex:
            return self.audit_chain_secret_hex.strip()
        if self.audit_chain_secret:
            return self.audit_chain_secret.encode("utf-8").hex()
        return None

    def resolve_evidence_signing_secret_hex(self) -> str | None:
        """Return the evidence HMAC key as a hex string, or None.

        Falls back to ``audit_chain_secret`` when the evidence-specific
        secret is not configured, so single-secret deployments still
        get signed receipts without extra env vars.
        """
        if self.evidence_signing_secret_hex:
            return self.evidence_signing_secret_hex.strip()
        if self.evidence_signing_secret:
            return self.evidence_signing_secret.encode("utf-8").hex()
        # Fallback: use the audit chain secret
        return self.resolve_audit_chain_secret_hex()
    ...
    def validate_common_runtime_settings(self) -> None:
        ...
        # The audit chain HMAC secret (migration 053) must be present in
        # production — without it the trigger refuses every insert into
        # claim_history, conflict_activity_log, accident_projection_history,
        # public_event_page_revisions, usage_events, and report_exports,
        # which would break ingestion and metering.  Outside production we
        # warn so dev environments work out of the box; the trigger itself
        # still requires a session GUC, so the UoW falls back to an
        # ephemeral dev secret.
        chain_hex = self.resolve_audit_chain_secret_hex()
        if chain_hex is None:
            if self.is_production:
                raise RuntimeError(
                    "AUDIT_CHAIN_SECRET (or AUDIT_CHAIN_SECRET_HEX) is required "
                    "when ENVIRONMENT=production. Generate one with "
                    "`openssl rand -hex 32`.  Rotate this secret only inside a "
                    "controlled window — it invalidates the historical chain."
                )
        elif self.is_production and not re.fullmatch(r"[0-9a-fA-F]{32,}", chain_hex):
            raise RuntimeError(
                "AUDIT_CHAIN_SECRET resolves to fewer than 16 bytes when "
                "ENVIRONMENT=production.  Use at least a 32-byte secret: "
                "`openssl rand -hex 32`."
            )

        # MFA KEK (migration 054).  Required in production so an operator
        # cannot enroll a key into MFA and then discover at first verify
        # that the seed cannot be decrypted.  Outside production, MFA seeds
        # may be left unencrypted-at-rest equivalent if no KEK is set —
        # ``atlas.security.mfa`` refuses to encrypt without one.
        mfa_kek_hex = self.resolve_mfa_kek_hex()
        if mfa_kek_hex is None:
            if self.is_production:
                raise RuntimeError(
                    "MFA_KEK (or MFA_KEK_HEX) is required when "
                    "ENVIRONMENT=production.  Generate one with "
                    "`openssl rand -hex 32`.  This key wraps per-API-key TOTP "
                    "seeds at rest; rotating it invalidates every enrolled "
                    "seed and requires re-enrollment."
                )
        elif not re.fullmatch(r"[0-9a-fA-F]{64}", mfa_kek_hex):
            raise RuntimeError(
                "MFA_KEK must resolve to exactly 32 bytes (64 hex characters) "
