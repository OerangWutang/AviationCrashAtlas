...
from atlas.domain.tenancy.entities import Tenant, TenantRole, TenantSource, TenantStatus
...
def _seed_tenant(uow, *, slug: str = "acme", is_active: bool = True) -> Tenant:
    t = Tenant(slug=slug, name=slug.upper(), status=TenantStatus.ACTIVE if is_active else TenantStatus.SUSPENDED)
