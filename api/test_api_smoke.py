...
async def test_provenance_include_archive_is_wired(async_client_reviewer):
    """include_archive is now implemented: it must not return the old 501 stub.

    An unknown event resolves to 404 (EventNotFoundError), proving the route
    reaches the use case rather than short-circuiting on include_archive.
    """
...
async def test_conflict_history_include_archive_is_wired(async_client_reviewer):
    """include_archive on conflict-history is accepted (no-op), never 501.

    Conflict-activity rows have no retention tombstone, so the flag is a no-op;
    an unknown conflict resolves to 404 rather than the old 501 stub.
    """
    resp = await async_client_reviewer.get(
...
async def test_provenance_archive_filters_soft_deleted_claims(
    async_client_reviewer, in_memory_uow
):
    """End-to-end: swept claims are hidden by default and surfaced via the flag.

    ``async_client_reviewer`` is wired to ``in_memory_uow``, so seeding the store
    here is what the request reads back.
    """
    from datetime import UTC, datetime

    from atlas.domain.entities import AccidentEvent, Claim, Source
    from atlas.domain.enums import ClaimType, SourceKind

    event_id = uuid4()
    source_id = uuid4()
    in_memory_uow._store.events[event_id] = AccidentEvent(id=event_id)
    in_memory_uow._store.sources[source_id] = Source(
        id=source_id, name="NTSB", kind=SourceKind.EXTERNAL
    )
    live_claim = Claim(
        event_id=event_id,
        source_id=source_id,
    ...
    in_memory_uow._store.claims[live_claim.id] = live_claim
    in_memory_uow._store.claims[swept_claim.id] = swept_claim

    default_resp = await async_client_reviewer.get(
        f"/api/v1/accidents/{event_id}/provenance"
    )
    assert default_resp.status_code == 200
    default_body = default_resp.json()
    default_ids = {c["id"] for c in default_body["claims"]}
    assert str(live_claim.id) in default_ids
    assert str(swept_claim.id) not in default_ids
    assert default_body["archive_available"] is True

    archive_resp = await async_client_reviewer.get(
        f"/api/v1/accidents/{event_id}/provenance?include_archive=true"
    )
    assert archive_resp.status_code == 200
    archive_ids = {c["id"] for c in archive_resp.json()["claims"]}
    assert str(live_claim.id) in archive_ids
    assert str(swept_claim.id) in archive_ids
