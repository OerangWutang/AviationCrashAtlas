...
class _EmptyResult:
    ...
    def scalar_one(self) -> int:
        # Used by the TICKET-009 lockout pre-check (counts failed attempts).
        # Returning 0 keeps the auth path from short-circuiting in unrelated
        # tests that don't care about brute-force protection.
        return 0
...
class _NoDbSession:
    ...
    def add(self, _obj) -> None:
        # TICKET-009 logs attempts via ``add(ApiKeyAttemptModel(...))``;
        # swallow it here so the no-DB session looks complete.
        return None
...
def _role_client(app, uow, role: Role):
    ...
    async def _tenant_uow_override(tenant_id: uuid.UUID):
        return uow
...
def make_tenant_client_for(
    ...
    *,
    ...
    if not any(
        m.tenant_id == tenant_id and m.atlas_user_id == fake_user.user_id
        for m in uow.store.tenancy.memberships
    ):
        uow.store.tenancy.memberships.append(
            TenantMembership(
                tenant_id=tenant_id,
                atlas_user_id=fake_user.user_id,
